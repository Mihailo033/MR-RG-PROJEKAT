...
void PlatformController::initialize() {
    if (glfwPlatformSupported(GLFW_PLATFORM_X11)) { glfwInitHint(GLFW_PLATFORM, GLFW_PLATFORM_X11); } else if (glfwPlatformSupported(GLFW_PLATFORM_WAYLAND)) { glfwInitHint(GLFW_PLATFORM, GLFW_PLATFORM_WAYLAND); } else if (glfwPlatformSupported(GLFW_PLATFORM_WIN32)) { glfwInitHint(GLFW_PLATFORM, GLFW_PLATFORM_WIN32); }
    ...
    // Ako imaš "fullscreen" u config.json
    if (config["window"].contains("fullscreen")) { isFullscreen = config["window"]["fullscreen"].get<bool>(); }

    GLFWwindow *handle = nullptr;

    if (isFullscreen) {
        // Uzmi primarni monitor i postavi rezoluciju
        GLFWmonitor *monitor = glfwGetPrimaryMonitor();
        const GLFWvidmode *mode = glfwGetVideoMode(monitor);
        handle = glfwCreateWindow(mode->width, mode->height, window_title.c_str(), monitor, nullptr);
    } else {
        // Windowed
        handle = glfwCreateWindow(window_width, window_height, window_title.c_str(), nullptr, nullptr);
    }
    ...
    for (int key = 0; key < m_keys.size(); ++key) { m_keys[key].m_key = static_cast<KeyId>(key); }
}
...
void PlatformController::poll_events() {
    ...
    glfwPollEvents();
    for (int i = 0; i < KEY_COUNT; ++i) { update_key(key_ref(static_cast<KeyId>(i))); }
}

void PlatformController::swap_buffers() { glfwSwapBuffers(m_window.handle_()); }

int glfw_platform_action(GLFWwindow *window, int glfw_key_code) {
    if (glfw_key_code >= GLFW_MOUSE_BUTTON_1 && glfw_key_code <= GLFW_MOUSE_BUTTON_LAST) { return glfwGetMouseButton(window, glfw_key_code); }
...
void PlatformController::update_key(Key &key_data) const {
    ...
    switch (key_data.state()) {
        case Key::State::Released: {
            if (action == GLFW_PRESS) { key_data.m_state = Key::State::JustPressed; }
            break;
        }
        case Key::State::JustReleased: {
            if (action == GLFW_PRESS) { key_data.m_state = Key::State::JustPressed; } else if (action == GLFW_RELEASE) { key_data.m_state = Key::State::Released; }
            break;
        }
        case Key::State::JustPressed: {
            if (action == GLFW_RELEASE) { key_data.m_state = Key::State::JustReleased; } else if (action == GLFW_PRESS) { key_data.m_state = Key::State::Pressed; }
            break;
        }
        case Key::State::Pressed: {
            if (action == GLFW_RELEASE) { key_data.m_state = Key::State::JustReleased; }
            break;
        }
    }
}

std::string_view Key::name() { return g_engine_key_to_string[m_key]; }
...
const MousePosition &PlatformController::mouse() const { return g_mouse_position; }

std::string_view PlatformController::name() const { return "PlatformGLFW3Controller"; }

void PlatformController::register_platform_event_observer(std::unique_ptr<PlatformEventObserver> observer) { m_platform_event_observers.emplace_back(std::move(observer)); }
...
void PlatformController::_platform_on_scroll(double x, double y) {
    ...
    for (auto &observer: m_platform_event_observers) { observer->on_scroll(g_mouse_position); }
}

void PlatformController::_platform_on_framebuffer_resize(int width, int height) {
    m_window.m_width = width;
    m_window.m_height = height;
    for (auto &observer: m_platform_event_observers) { observer->on_window_resize(width, height); }
}

void PlatformController::_platform_on_window_close(GLFWwindow *window) { if (m_window.handle_() == window) { glfwSetWindowShouldClose(window, GLFW_TRUE); } }
...
void PlatformController::set_enable_cursor(bool enabled) { if (enabled) { glfwSetInputMode(m_window.handle_(), GLFW_CURSOR, GLFW_CURSOR_NORMAL); } else { glfwSetInputMode(m_window.handle_(), GLFW_CURSOR, GLFW_CURSOR_DISABLED); } }
...
static void glfw_mouse_callback(GLFWwindow *window, double x, double y) { core::Controller::get<PlatformController>()->_platform_on_mouse(x, y); }

void glfw_mouse_button_callback(GLFWwindow *window, int button, int action, int mods) { core::Controller::get<PlatformController>()->_platform_on_mouse_button(button, action); }
...
static void glfw_key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) { core::Controller::get<PlatformController>()->_platform_on_keyboard(key, action); }
...
void glfw_window_close_callback(GLFWwindow *window) { core::Controller::get<PlatformController>()->_platform_on_window_close(window); }
